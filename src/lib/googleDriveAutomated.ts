...
// This will automatically fetch images from your Google Drive folder
...
class AutomatedGoogleDriveService {
  ...
  /**
   * Automatically fetch all images from your Google Drive folder
   */
  async fetchImages(): Promise<GoogleDriveImage[]> {
    try {
      console.log('🔄 Automatically fetching images from Google Drive...');
      
      // Query to get all image files from the folder
      const query = `'${this.folderId}' in parents and mimeType contains 'image/'`;
      const url = `${this.baseUrl}/files?q=${encodeURIComponent(query)}&fields=files(id,name,mimeType,size,createdTime,webViewLink)&key=${this.apiKey}`;
      ...
      // Transform the data to our format with multiple URL formats for better compatibility
  ...
  /**
   * Get fallback images if Google Drive API fails
   */
...
export const automatedGoogleDriveService = new AutomatedGoogleDriveService();
